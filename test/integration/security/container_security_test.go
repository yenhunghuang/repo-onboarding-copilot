...
func isDockerAvailable() bool {
	// Try multiple Docker commands with increasing complexity
	commands := [][]string{
		{"docker", "version"},
		{"docker", "info"},
		{"docker", "ps", "-q"},
	...
	for _, cmdArgs := range commands {
		...
		// Always inherit the full environment to ensure DinD variables are passed
		cmd.Env = os.Environ()
	...
	// If all commands failed, Docker is not available
	return false
}
